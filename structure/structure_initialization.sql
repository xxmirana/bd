CREATE TABLE Departments (
    DepartmentID SERIAL PRIMARY KEY,  -- автоматическая генерация идентификаторов с применением SERIAL
    DepartmentName VARCHAR(100) NOT NULL
);

CREATE TABLE Roles (
    RoleID SERIAL PRIMARY KEY,  -- автоматическая генерация идентификаторов с применением SERIAL
    RoleName VARCHAR(100) NOT NULL
);

CREATE TABLE Employees (
    EmployeeID SERIAL PRIMARY KEY,  -- автоматическая генерация идентификаторов с применением SERIAL
    Name VARCHAR(100) NOT NULL,
    Position VARCHAR(100),
    ManagerID INT,
    DepartmentID INT,
    RoleID INT,
    FOREIGN KEY (ManagerID) REFERENCES Employees(EmployeeID) ON DELETE SET NULL,  -- установка поведения при удалении
    FOREIGN KEY (DepartmentID) REFERENCES Departments(DepartmentID) ON DELETE CASCADE,  -- установка поведения при удалении
    FOREIGN KEY (RoleID) REFERENCES Roles(RoleID) ON DELETE SET NULL  -- установка поведения при удалении
);

CREATE TABLE Projects (
    ProjectID SERIAL PRIMARY KEY,  -- автоматическая генерация идентификаторов с применением SERIAL
...
CREATE TABLE Tasks (
    TaskID SERIAL PRIMARY KEY,  -- автоматическая генерация идентификаторов с применением SERIAL
    TaskName VARCHAR(100) NOT NULL,
    AssignedTo INT,
    ProjectID INT,
    FOREIGN KEY (AssignedTo) REFERENCES Employees(EmployeeID) ON DELETE SET NULL,  -- установка поведения при удалении
    FOREIGN KEY (ProjectID) REFERENCES Projects(ProjectID) ON DELETE CASCADE  -- установка поведения при удалении
);
-- Добавление отделов
